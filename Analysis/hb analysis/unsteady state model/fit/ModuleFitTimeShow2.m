function ModuleFitTimeShow2(DataCellMean,TimeLabelMean,DataCell,TimeLabel,RnaLenth,kfitTime,KannealG,PiniG,OutFolder,InheritSwitch,RepeatNum,Bound,Model)
...
for T_i = 1:TimeNum
    ...
    if 1
        ...
        area(0:70,Pdtb(1:71),'FaceColor','r','FaceAlpha',0.3) 
        ylim([0 0.05]);xlim([-0.5 70])
        xlabel('RNA Number')
        ylabel('Frequency')
        axis square
        text(40,0.045,['T = ',num2str(TimeShowSort(T_i)),' sec'],'Color','b','FontSize',18)
        title(['Evolutionary process'],'Color',DataLabelColor{TimeShowLabelSort(T_i)+1})
        F=getframe(gcf);
        I=frame2im(F);
        [I,map]=rgb2ind(I,256);
        if T_i == 1
            imwrite(I,map,[OutFolder,'TimeEvoMean',InheritText{InheritSwitch+1},'.gif'],'gif','Loopcount',inf,'DelayTime',DelayTime(TimeShowLabelSort(T_i)+1));
        else
            imwrite(I,map,[OutFolder,'TimeEvoMean',InheritText{InheritSwitch+1},'.gif'],'gif','WriteMode','append','DelayTime',DelayTime(TimeShowLabelSort(T_i)+1)); 
        end
        if T_i>=CatLim(1)&&T_i<=CatLim(2)
            if T_i == CatLim(1)
                imwrite(I,map,[OutFolder,'TimeCatEvoMean',InheritText{InheritSwitch+1},'.gif'],'gif','Loopcount',inf,'DelayTime',DelayTime(TimeShowLabelSort(T_i)+1));
            else
                imwrite(I,map,[OutFolder,'TimeCatEvoMean',InheritText{InheritSwitch+1},'.gif'],'gif','WriteMode','append','DelayTime',DelayTime(TimeShowLabelSort(T_i)+1)); 
            end
        end
