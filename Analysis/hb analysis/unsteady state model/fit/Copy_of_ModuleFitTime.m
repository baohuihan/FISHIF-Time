...
for T_i = 1:TimeNum
    ...
    if FitSwitch
        ...
        if T_i==1
            ...
            if Kindex(1)==1
                PiniStart0=PiniG(K_i-1,:)';%Pini
                TimeStart0=TimeLabel(K_i-1,:);
            else
                PiniStart0=[1 0 0]';
                TimeStart0=-Tstay;
            end
            [~,~,~,PiniStart,~] =  FSP_FreeSet(KannealG(Kindex(1),:),0,PiniStart0,LabelPart,0,Kstart,UnLockIndex,TimeElong(1)-TimeStart0,[]);
            EvoI=0;
            LabelUse=[];
            LabelWait=Label;
            for K_i=Kindex 
