function TimeFitMainTest(Cycle,Model,TimeBin,InheritSwitch,CombinNum)
...
% TimeBin=0.5;
% InheritSwitch=0;
LabelTip='-EL[0.2 0.4]bin0.1Move0.05-TimeWindowMingle-Vel40-KL[0.01 0.1 0.60]-KU[0.2 0.1 0.8]-TimeStart60-KDisLost[10 10 1 1 1]-Artifical-Filter5-timeBinMove1-PonBoole0-re6-0514\';
RepeatNum=1;
...
switch Model
    case 'Ton'
        Bound=[...
            [1.00 0 1.00 0 0 0 0 1.00 0 1 0 1];...
            [0.20,0,0.1,0,0,0,0,0.80,0,1,1,1];...
            [0.01,0,0.1,0,0,0,0,0.60,0,0,1,0]];
    case 'TwoState'
        Bound=[...
...
switch Cycle
    ...
    case '12'
        ...
        artificalFilter=[2 35 36];
        artificalFilterHold=[1 8 34];
        artificalTimeCombin=[];
        % CombinPart=[5 2 3];
        CombinPart=[4 2 2 2];
        StdDel=25;
...
for ELi=1:size(EL,2)
    ...
    EmPathSortInput=EmPathSort(~ErrorIndex);
    %% show data%%%Time Fit
    ...
    RnaSignalExpPartMean=cell2mat(cellfun(@(x) mean(x(x~=0)),RnaSignalInput,'UniformOutput',false));
    figure
    for ii=1:length(RnaSignalInput)
        nexttile;
        histogram(RnaSignalInput{ii},0:1:100,'Normalization','probability')
        ylim([0 0.05]);xlim([-0.5 100])
        title(['Peak Mean: ',num2str(RnaSignalExpPartMean(ii))])
    end
    figure
        histogram(RnaSignalInput{25},0:1:100,'Normalization','probability')
        ylim([0 0.15]);xlim([-0.5 100])
   figure
        histogram(RnaSignalInput{16},0:1:100,'Normalization','probability')
        ylim([0 0.15]);xlim([-0.5 100])
