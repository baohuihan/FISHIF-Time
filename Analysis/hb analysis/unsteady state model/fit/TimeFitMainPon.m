...
OutFolderMain=...
    ['Y:\TimeFit\Output\CDS\Cycle13\Inherit-TwoPon-Turn1-TimeBin2-IniModel2-EL[0.2 0.4]bin0.2-ReduceModel2-SetPonFree-',...
    'Lock[Kini 0.25 0.55][Koff 0.1][Alp 0 1]\'];
mkdir(OutFolderMain);
...
TimeInterval=TimeLabelMean(2:end)-TimeLabelMean(1:end-1);
Interval=0.8;
while sum(TimeInterval<Interval)>=1
    IndexI=TimeIndex(TimeInterval<Interval);
    TimeLabelMean(IndexI(1))=mean(TimeLabelMean(IndexI(1):IndexI(1)+1),2);
    TimeLabelMean(IndexI(1)+1)=[];
    DataCellMean{IndexI(1)}=cat(1,DataCellMean{IndexI(1)},DataCellMean{IndexI(1)+1});
    DataCellMean(IndexI(1)+1)=[];
    TimeInterval=TimeLabelMean(2:end)-TimeLabelMean(1:end-1);
end
MeanExp=MeanPlot(TimeLabelMean*60,DataCellMean,'');%data combin
xlabel('Time/sec');ylabel('MeanExpr');xlim([0 max(TimeLabelMean*60)+60]);title(ELlabel)
saveas(gcf,[OutFolder,'Mean-Time-combine-TimeInterval','.png']);

[KannealG,MeanExpFitG,PiniG]=ModuleFitTimeInherit(DataCellMean,TimeLabelMean,OutFolder);
