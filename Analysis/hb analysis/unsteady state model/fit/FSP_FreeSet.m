function [lld,varargout] = FSP_FreeSet(Kanneal,Kanneal0,ndata,p_ini,dna_label,hold_time,Kall,AnnealLogic,Tstay,DtdInherit,Kdiff,varargin)
%% A program to calculate the likelihood of 3 state model

%% Parameter setting and initialization
Kall(AnnealLogic)=Kanneal;
Kall0(AnnealLogic)=Kanneal0;
k_all=Kall;
...
IniP=P_dtb_1(1:Nstate,1)./sum(P_dtb_1(1:Nstate,1),1);
NolockP=P_dtb_1;

PonChange=GeneActiveR-Pon0;
if PonChange>=0
    P_dtb_1_Locked = zeros(Nstate*Nx,1);
    P_dtb_1_Locked(1:Nstate,1)=(1-GeneActiveR)*IniP;
    P_dtb_1=P_dtb_1-P_dtb_1_Locked;
elseif PonChange<0
    P_dtb_1_Locked1 = zeros(Nstate*Nx,1);
    P_dtb_1_Locked1(1:Nstate,1)=(1-Pon0)*IniP;
    P_dtb_1=P_dtb_1-P_dtb_1_Locked1;
    NolockP=P_dtb_1./sum(P_dtb_1,1);
    P_dtb_1_Locked2=-PonChange*NolockP;
    P_dtb_1=P_dtb_1-P_dtb_1_Locked2;
    P_dtb_1_Locked=P_dtb_1_Locked1+P_dtb_1_Locked2;
end
%% post probe
rna_signal1=Nbin;
k_smat1_use = k_smat1;
...
if Pon0==0 && mean(ndata(:))<0.2
    ndata(ndata>25)=0;
end
%% likelihood
if Kdiff==1
    Kdistance=1e-1*dist([10,5,0,1.2,0]'.*[Kanneal(1,1:5)',Kanneal0(1,1:5)']);
else
    Kdistance=[0 0];
end
p_tot_bin_log=log(P_dtb_rna_use);
lld=-sum(p_tot_bin_log(round(ndata)+1))/length(ndata)+Kdistance(1,2);
% lld=-(sum(p_tot_bin_log(round(ndata)+1))/length(ndata)+Kdistance(1,2))*length(ndata);
% Nbin_data=0:1:100;
