function [KannealG,MeanExpFitG]=ModuleFitTime_Ton(RnaSignal,TimeLabel,OutFolder)
...
for T_i = 1:TimeNum
    ...
    Ub=[0.05,0,0.05,0,0,0,0,0.55,0,1,1,1];
    Lb=[0.05,0,0.05,0,0,0,0,0.25,0,0,1,0];
...
%         saveas(gcf,[OutFolder,['Pioss-Time',num2str(TimeLabel(T_i)),'.png']]);
        ...
        if T_i==1%%First evo
            ...
            switch IniModel
                case 1
                    ...
                    [~,~,~,PiniStart,~] =  FSP_FreeSet(KannealG(KIStart,:),0,PiniStart0,LabelPart,0,Kstart,UnLockIndex,TimeElong(1)-TimeStart0,[]);%%MaybeErrorExist
                case 2
        ...
        while Turn < RepeatNum  % repeat turn
            ...
            [Kanneal,~,~] = simulannealbnd(@(x) FSP_FreeSetTon(x,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit),...
                KstartAnneal,LbAnneal,UbAnneal,...
                 options);
            %% Lld&Pini
            [lld,~,~,pini,~] =  FSP_FreeSetTon(Kanneal,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit);
            if lld<=LldPilot
            ...
            LldG(T_i,:)=LldPilot;
            [~,Pdtb,~,Pinis,~] =  FSP_FreeSetTon(KannealOut,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit);%Get Distribution
...
%             [~,Pdtb_Hold,~,~,~] =  FSP_FreeSetTon(KannealHoldOff,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit);
            [~,Pdtb_steady,~,~,~] =  FSP_FreeSetTon_Steady(KannealOut,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit);
