function [KannealG,MeanExpFitG,PiniG]=ModuleFitTimeInherit(RnaSignal,TimeLabel,OutFolder)
...
for T_i = 1:TimeNum
    ...
    Lb=[0.001,0,0.1,0,0,0,0,0.25,0,0,1,0];
...
%             end
            ...
            if InheritSwitch==1%Inherit of Rna distribution
                ...
                for K_i=Kindex 
                    ...
                    [~,~,~,Pinie,DtdInherit] =  FSP_FreeSetPon2(KannealG(K_i,:),0,Pinie,LabelPart,0,Kstart,UnLockIndex,TimeAtKev,Change,DtdInherit);
                end
        ...
        while Turn < RepeatNum  % repeat turn
            ...
            options = optimoptions('simulannealbnd','MaxFunctionEvaluations',100000);
...
% %             end
            ...
            if lld<=LldPilot
                ...
                PiniG(T_i,:)=pini;
                if abs(sum(pini)-Kanneal(4))>1e-2
                    diag('Error')
                end
                LldPilot=lld;
                KannealOut=Kanneal;
            ...
            if ~isempty(DtdInherit)
                P_dtb_all=reshape(DtdInherit,[3,101]);
                P_dtb_rna = sum(P_dtb_all);
                P_dtb_nx=reshape(P_dtb_rna(2:end),[1,100]);
...
%             [~,Pdtb_Hold,~,~,~] =  FSP_FreeSetTon(KannealHoldOff,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit);
        ...
        plot(0:70,PInherit(1:71),'--r','LineWidth',2)
        xlabel('RNA Number')
        ylabel('Frequency')
        axis square
