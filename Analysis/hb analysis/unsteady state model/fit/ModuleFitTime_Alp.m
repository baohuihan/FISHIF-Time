function [KannealG,MeanExpFitG]=ModuleFitTime_Alp(RnaSignal,TimeLabel,OutFolder)
...
for T_i = 1:TimeNum
    ...
    Ub=[0.2,0,0.2,0,0,0,0,0.55,0,0,1,1];
...
%         saveas(gcf,[OutFolder,['Pioss-Time',num2str(TimeLabel(T_i)),'.png']]);
        ...
        while Turn < RepeatNum  % repeat turn
            ...
            [~,Pdtb_Hold,~,~,~] =  FSP_FreeSet(KannealHoldOff,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit);
            [~,Pdtb_steady,~,~,~] =  FSP_FreeSet_Steady(KannealOut,DataTime,Pinie,LabelPart,0,Kstart,UnLockIndex,Tfit,DtdInherit);
