function [lld,varargout] = FSP_threestate(k_all,ndata,p_ini,dna_label,hold_time,varargin)
%% A program to calculate the likelihood of 3 state model

%% Parameter setting and initialization
% dna_label=[497,2725,3+283+2277+407+509];
...
P_dtb_rna_use=P_dtb_rna(1:61);
P_dtb_rna_use(1)=P_dtb_rna_use(1)+sum(P_dtb_rna_use(2:4));
% P_dtb_rna_use(2:3)=0;
ndata(ndata >= 60.5)=0;
ndata(ndata < 3.5)=0;
%% likelihood
p_tot_bin_log=log(P_dtb_rna_use);
lld=-sum(p_tot_bin_log(round(ndata)+1))+k_all(4)/k_all(6)*0;
% Nbin_data=0:1:100;
% nf = histc(reshape(ndata,1,length(ndata)),Nbin_data);
% nf=nf'/sum(nf);
% lld=-sum(1./abs(P_dtb_rna(1:61)-nf(1:61)'));
varargout={P_dtb_rna,P_dtb_all,p_ini};