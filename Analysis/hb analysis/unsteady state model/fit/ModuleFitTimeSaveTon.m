function ModuleFitTimeSaveTon(TimeLabelMean,EL,KannealELs,PiniELs,MeanExpELs,OutFolderMain,Color)
Kplot=struct('ON',[],'OFF',[],'Kon',[],'Koff',[],'Sf',[],'S0',[],'S1',[],'Kratio',[],'Kini',[],'Alpha',[],'MeanData',[],'MeanFit',[],'MeanFitSteady',[],'OnRatio',[],'ActiveR',[]);
for ELi=1:size(EL,2)
    KannealG=KannealELs{ELi,1};
    TimeLabelMeanel=TimeLabelMean{ELi,1};
    PiniG=PiniELs{ELi,1};
    ...
    Kplot.ActiveR=[Kplot.ActiveR,KannealG(:,4)];
...
for ELi=1:size(EL,2)
    ...
    plot(Ytime,Kplot.ActiveR(:,ELi),'-o','LineWidth',2,'color','c')
    xlabel('Time/sec');title(ELlabel)
    ylim([0 1])
    legend('Soff','S0','S1','Active')
end
